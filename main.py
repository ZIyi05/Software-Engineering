...
def update_profile():
    ...
    try:
        ...
        if not (0.0 <= cgpa_val <= 4.0):
            flash("Error: CGPA must be between 0.0 and 4.0")
            return redirect(url_for('profile'))
    except (ValueError, TypeError):
        flash("Error: Invalid CGPA format")
        return redirect(url_for('profile'))
    # ----------------------------

    # Proceed with database update if valid
    ...
    try:
        with connection.cursor() as cur:
            ...
            cur.execute("UPDATE student SET faculty=%s, course=%s, cgpa=%s, total_credits=%s WHERE studentID=%s", (faculty, course, cgpa_val, credits, uID))
        connection.commit()
